...
const Menu = () => {
  ...
  const PriceComponent: React.FC<PriceComponentProps> = ({ price, categoryType }) => {
    let priceValue = `${Validate.unit(Number(price))}đ`;
    if (categoryType === 'hotPot') {
      const numericPrice = Number(price);
      priceValue = `${Validate.unit(numericPrice)}đ - ${Validate.unit(numericPrice + 70000)}đ`;
    }
    return <p id="menu-item--price">{priceValue}</p>;
  };
  ...
  return (
    <div className="menu">
      {/* <Image
        className="banner"
        loading="eager"
        src={isHighResLoaded ? "/images/menu.png" : "/images/menu-banner.svg"}
        alt="menu"
        width={1708}
        height={969}
        style={{ width: "100%", height: "auto" }}
        onLoad={(e) => setHighResLoaded(true)}
      /> */}
